...
class Neuron{
    constructor(size, no){
        ...
        this.dW = [];
        this.weights = [];
        this.negativeWeights = true;
        this.InitializeNeuron();
        this.SetActivationFunction(signum);
        //this.setRandomActivation();
    }
    setRandomActivation(){
        var random = getRandomInteger(1,5);
        ...
        else if(random == 4) this.SetActivationFunction(signum);
    ...
    InitializeNeuron(){
        for(var i=0;i<this.size;i++){
            if(this.negativeWeights == true){
                if(getRandomInteger(0,1) == 0) this.weights.push(Math.random() * 0.01 * -1);
                else this.weights.push(Math.random() * 0.01);
            }else{
...
class GeneticAlgorithm{
    constructor(populationSize, networkDimensions){
        ...
        this.percentPopulationPreserve = 4;
        this.mutationRate = 0.35;
        this.generationNumber = 0;
        this.randomNewNetworks = 25;
    ...
    SortByFitness(){
        //for(var i=0;i<this.populationSize;i++) console.log(this.population[i].fitness + " " + this.population[i].network.layers[0].neurons[0].weights[2] );
    ...
    setPercentPopulationPreserve(value){ this.percentPopulationPreserve = value; }
    ...
    selectionAndBreeding(){
        ...
        p = Math.floor(p);
        r = Math.floor(r);
        //console.log(this.population[this.populationSize - 1].network.layers[0].neurons[0].weights[2]);
        newPopulation = this.population.splice(this.populationSize - p);
        //console.log(newPopulation[p - 1].network.layers[0].neurons[0].weights[2]);
        ...
        for(var i=0; i<p; i++){
            if(newPopulation[i].fitness == 0){
                newPopulation[i].network = new NeuralNetwork(this.networkDimensions);
            }
        }
        ...
        for(var i=p; i<this.populationSize - r; i++){
            ...
            for(var j = 0; j < this.networkDimensionSize; j++){
                for(var k =0; k < parent1.network.layers[j].neurons.length; k++){
                    randomness = getRandomInteger(0,100);
                    if(randomness <= 10){
        ...
        //console.log(this.population[p - 1].network.layers[0].neurons[0].weights[2]);
    }
                            ...
                            layer.neurons.forEach((neuron)=>{
                                if(Math.random() < Math.random()){
                                    for(var i=0;i<neuron.size;i++){
                                        if(this.negativeWeights == true){
                                            if(getRandomInteger(0,1) == 0) neuron.weights[i] += (Math.random() * -1 * 0.1);
                                            else neuron.weights[i] += (Math.random() * 0.1);
                                        }else{
                                            neuron.weights[i] +=( Math.random() * 0.1 );
                                        }
                                    }
                                }
                            });
